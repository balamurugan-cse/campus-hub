...
export const SignInPage: React.FC<SignInPageProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!cleanEmail) {
        throw new Error('Please enter your college email or Register / Roll Number.');
      }

      // If user typed a register number or username without domain (e.g., 310621104001 or 21cs045)
          ...
          <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={role === 'student' ? 'e.g. 310621104001 or student@dmice.ac.in' : 'faculty@dmice.ac.in'}
                  className="w-full pl-10 pr-3.5 py-2.5 text-sm rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-purple-600/30 focus:border-purple-600"
                />
              </div>
              <p className="text-[10px] text-slate-400 mt-1">
                Accepts DMICE institutional email, personal Gmail, or Anna Univ. Register Number.
              </p>
            </div>
            ...
            <div className="pt-3 border-t border-slate-100">
              <span className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2 text-center">
                Quick Campus Access (1-Tap Fill)
              </span>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <button
                  ...
                  onClick={() => {
                    setRole('student');
                    setEmail('palanibalamurugan818@gmail.com');
                    setPassword('Bala@1202');
                    setError(null);
                  }}
                  className="p-2 rounded-xl bg-purple-50 hover:bg-purple-100 border border-purple-200/80 text-left transition-colors cursor-pointer"
                >
                  <span className="block text-[11px] font-bold text-purple-900 flex items-center gap-1">
                    <GraduationCap className="w-3.5 h-3.5 text-purple-600" />
                    Student Bala Murugan
                  </span>
                  <span className="block text-[10px] text-purple-600 font-mono mt-0.5 truncate">
                    palanibalamurugan818@gmail.com
                  </span>
                </button>
                ...
                <button
                  ...
                  onClick={() => {
                    setRole('organizer');
                    setEmail('faculty.cse@dmice.ac.in');
                ...
                >
                  <span className="block text-[11px] font-bold text-slate-800 flex items-center gap-1">
                    <ShieldCheck className="w-3.5 h-3.5 text-slate-600" />
                    Faculty Coordinator
                  </span>
                  <span className="block text-[10px] text-slate-500 font-mono mt-0.5 truncate">
                    faculty.cse@dmice.ac.in
                  </span>
                </button>
              </div>
            </div>
